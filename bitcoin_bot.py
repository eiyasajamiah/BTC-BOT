...
# AGGREGATOR
# ══════════════════════════════════════════════════════════════════
def aggregate(v7: dict, smc: dict, ob: dict, asia: dict = None, rng: dict = None, fib: dict = None) -> dict:
    if asia is None: asia = {}
    if rng  is None: rng  = {}
    if fib is None: fib = {}
    votes={"V7":v7["vote"],"SMC":smc["vote"],"OBinOB":ob["vote"],"Asia":asia.get("vote",0),"Range":rng.get("vote",0),"Fib":fib.get("vote",0)}
...
def _save_results(v7, smc, ob, asia, rng, agg, price, fib=None):
    global _last_results, _last_results_ts
    _last_results = {"v7":v7,"smc":smc,"ob":ob,"asia":asia,"range":rng,"agg":agg,"price":price,"fib":fib or {}}
...
def main():
    ...
    while True:
        ...
        agg=aggregate(v7,smc,ob,asia,rng_res,fib_res)
        ...
        _save_results(v7,smc,ob,asia,rng_res,agg,smc.get("price",cur_price),fib_res)
