...
# AGGREGATOR
# ══════════════════════════════════════════════════════════════════
def aggregate(v7: dict, smc: dict, ob: dict, asia: dict = None, rng: dict = None, fib: dict = None, rev: dict = None) -> dict:
    if asia is None: asia = {}
    if rng  is None: rng  = {}
    if fib is None: fib = {}
    if rev  is None: rev  = {}
    votes={"V7":v7["vote"],"SMC":smc["vote"],"OBinOB":ob["vote"],"Asia":asia.get("vote",0),"Range":rng.get("vote",0),"Fib":fib.get("vote",0),"Rev":(rev.get("vote",0) if rev else 0)}
    ...
    return {"d":d,"agree":ag_cnt,"conf":conf,"votes":votes,"lbl":lbl,
            "entry":entry,"sl":sl,"tp":tp,"tp1":tp1,"tp2":tp2,"tp3":tp3,"rr":rr,
            "asia":asia,"range":rng,"fib":fib,"rev":rev}
...
def _save_results(v7, smc, ob, asia, rng, agg, price, fib=None, rev=None):
    global _last_results, _last_results_ts
    _last_results = {"v7":v7,"smc":smc,"ob":ob,"asia":asia,"range":rng,"agg":agg,"price":price,"fib":fib or {},"rev":rev or {}}
...
def _build_keyboard(signal_dir: int = 0) -> dict:
    ...
    return {"inline_keyboard":[
        ...
        [btn("🔄 أنماط الانعكاس","analyze_rev"),     btn("🤖 Claude AI","analyze_ai")],
        [btn(f"📈 ملخص {arrow}","analyze_full")],
    ]}
...
def msg_analysis(v7: dict, smc: dict, ob: dict, price: float, asia: dict = None, rng: dict = None, fib: dict = None) -> str:
...
def main():
    ...
    while True:
        ...
        agg=aggregate(v7,smc,ob,asia,rng_res,fib_res,rev_res)
        ...
        _save_results(v7,smc,ob,asia,rng_res,agg,smc.get("price",cur_price),fib_res,rev_res)
        ...
        if agg["d"]!=0 and agg["entry"] is not None:
            ...
            if sid==last_sig:
                ...
                if not sq_res.get("tradeable",True):
                    log.info(f"[SQ] إشارة مهملة {sq_res['score']}/100")
                    _send1(str(TG_CHAT),f"⚠️ إشارة {agg['lbl']} مهملة — جودة {sq_res['score']}/100 ({sq_res['grade']})")
                    continue
            ...
            ana_txt=msg_analysis(v7,smc,ob,smc.get("price",cur_price),asia,rng_res,fib_res)
