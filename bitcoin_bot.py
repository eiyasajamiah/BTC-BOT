...
TRIG_FM   = int(cfg.get("TRIGGER_FORCE_EVERY_MIN", 5))
...
_AI_PROVIDERS = {
    ...
    "ppq": {
        ...
        "model":   "claude-sonnet-4-5",   # PPQ يدعم نفس نماذج Anthropic
        "headers": lambda key: {
            "Authorization":  f"Bearer {key}",   # OpenAI-compatible format
            "content-type":   "application/json",
        },
    },
}
...
def ai_analyze(v7: dict, smc: dict, ob: dict, agg: dict, price: float) -> dict:
    ...
    log.info(f"[AI] جاري الإرسال إلى {provider_name.upper()} ({api_url})...")
...
# AGGREGATOR
# ══════════════════════════════════════════════════════════════════
def aggregate(v7: dict, smc: dict, ob: dict, asia: dict = None) -> dict:
    if asia is None: asia = {}
    votes={"V7":v7["vote"],"SMC":smc["vote"],"OBinOB":ob["vote"],"Asia":asia.get("vote",0)}
...
def _pu_prime_ad(signal_dir: int = 0) -> str:
    ...
    return (
        "\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🏦 <b>سجّل الآن في PU Prime وتداول الذهب باحترافية</b>\n\n"
        "✨ <b>لماذا PU Prime؟</b>\n"
        "  💰 <b>بونص 50%</b> على الإيداع الأول\n"
        "  📊 سبريد من 0.0 pip على الذهب BTC/USD\n"
...
def msg_signal(v7: dict, smc: dict, ob: dict, agg: dict, trigger_reasons: list) -> str:
    ...
    lines=[
        f"{'📈' if d==1 else '📉'} <b>إشارة ذهب موحّدة v2.0 — BTC/USD</b>",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
...
def msg_analysis(v7: dict, smc: dict, ob: dict, price: float, asia: dict = None) -> str:
...
def msg_daily(perf: dict, price: float) -> str:
    lines=[
        "📈 <b>التقرير اليومي — بوت البيتكوين الموحّد v2.0</b>",
...
def main():
    log.info("="*65)
    log.info("🚀 بوت البيتكوين الموحّد v2.0 — يبدأ التشغيل")
        ...
        if agg["d"]!=0 and agg["entry"] is not None:
            ...
            if sid==last_sig:
                ...
                try:
                    ...
                    if ch:
                        sent=tg_photo(ch,cap=txt,flt=S_SIG)
                        if not sent: tg_send(txt,flt=S_SIG)
                    else: tg_send(txt,flt=S_SIG)
                except: log.exception("Chart error"); tg_send(txt,flt=S_SIG)
                dr="شراء 🟢" if agg["d"]==1 else "بيع 🔴"
                log.info(f"✅ {dr} توافق={agg['agree']}/3 ثقة={agg['conf']}% RR={agg['rr']} دخول={agg['entry']:.2f}")
                last_sig=sid
        else:
            tg_send(msg_analysis(v7,smc,ob,smc.get("price",cur_price),asia),flt=S_ANA)
