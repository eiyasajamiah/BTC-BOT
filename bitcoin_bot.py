...
# -*- coding: utf-8 -*-
"""
bitcoin_bot.py  —  بوت البيتكوين الموحّد v1.0
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
# CONFIG
# ══════════════════════════════════════════════════════════════════
CFG_FILE = "btc_config.json"
DEF: Dict = {
    "TELEGRAM_TOKEN": "", "TELEGRAM_CHAT_ID": "",
    "SYMBOL": "BTC-USD", "DXY_SYMBOL": "DX-Y.NYB",
    "RUN_INTERVAL_MIN": 1,
    ...
    "OB_MIN_RR": 1.8,
    # Unified
    "UNIFIED_MIN_AGREE": 2, "PERF_FILE": "btc_performance.json",
    # Smart Trigger
    "TRIGGER_PRICE_CHANGE_PCT": 0.15,
    "TRIGGER_VOLUME_SPIKE_X": 2.0,
    ...
    "CLAUDE_PROVIDER": "auto",  # auto=مجاني أولاً | groq | openrouter | gemini | ppq | anthropic
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("logs/bitcoin.log", encoding="utf-8"),
    ],
)
log = logging.getLogger("BitcoinBot")

# ══════════════════════════════════════════════════════════════════
# TELEGRAM
# ══════════════════════════════════════════════════════════════════
SUBS_FILE = "btc_subscribers.json"
...
def _handle_cmd(cid: str, nm: str, txt: str):
    cmds = {
        "/start":       (S_ALL,
            "🟠 <b>أهلاً بك في بوت البيتكوين الموحّد!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    ...
    if txt in ("/start", "/subscribe"):
        ...
        _send1(cid,
            "✨ <b>بوت البيتكوين الموحّد — BTC/USD</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "🟠 <b>ما هو هذا البوت؟</b>\n"
        ...
        _send1(cid,
            "💛 <b>دعم مطوّر البوت</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "شكراً لاستخدامك بوت البيتكوين الموحّد 🟠\n\n"
...
STRAT_NAMES = [
    ...
    "Fib_Extension",
    "Fib_GoldenPocket",
    "Rev_Hammer","Rev_Engulfing","Rev_MorningStar",
    "Rev_DoubleTB","Rev_HeadShoulders","Rev_Wedge",
    "SC_Marubozu","SC_Hammer","SC_Doji","SC_Strong","SC_BeltHold",
]
...
OB_SIG_FILE="btc_ob_signals.json"
...
ASIA_MIN_IMPULSE_PIPS = 50.0  # أدنى حركة impulse بالدولار  # أدنى حركة impulse بالدولار
...
# ══════════════════════════════════════════════════════════════════
# MODULE 6 — Fibonacci Analysis (استراتيجيات فيبوناتشي الشاملة)
# ══════════════════════════════════════════════════════════════════
#
# الاستراتيجيات المدمجة:
#   ① Retracement    — مستويات التصحيح (23.6% → 78.6%)
#   ② Extension      — أهداف الامتداد (127.2% → 261.8%)
#   ③ Fan Lines      — خطوط المروحة (دعم/مقاومة ديناميكية)
#   ④ Time Zones     — المناطق الزمنية (1,2,3,5,8,13,21)
#   ⑤ Arcs           — الأقواس (مناطق دائرية)
#   ⑥ Channel        — قناة فيبوناتشي
#   ⑦ Pivot + Fib    — بيفوت مدمج مع فيبوناتشي
#   ⑧ Golden Pocket  — المنطقة الذهبية (61.8%–65%)
#   ⑨ Confluence     — تقاطع مستويات متعددة

FIB_LOOKBACK     = 100   # عدد الشموع لتحديد الموجة
FIB_TOUCH_BUF    = 0.003 # هامش اللمس 0.3%
FIB_CONFLUENCE   = 3     # عدد التقاطعات لتأكيد المستوى
FIB_MIN_WAVE     = 0.005 # أدنى حجم موجة 0.5%
FIB_GOLDEN_LO    = 0.618
FIB_GOLDEN_HI    = 0.650
...
FIB_RETRACE_LEVELS = {
    ...
    "61.8%": 0.618, "65.0%": 0.650, "78.6%": 0.786,
}
FIB_EXTEND_LEVELS = {
    "100%":   1.000, "127.2%": 1.272, "141.4%": 1.414,
    "161.8%": 1.618, "200%":   2.000, "261.8%": 2.618,
}


def _detect_wave(df: pd.DataFrame) -> Optional[dict]:
    """
    يكشف الموجة الرئيسية (Swing High → Swing Low أو العكس)
    يُرجع: {direction, wave_hi, wave_lo, wave_start, wave_end, size_pct}
    """
    sub  = df.tail(FIB_LOOKBACK)
    hi   = float(sub["High"].max())
    lo   = float(sub["Low"].min())
    size = (hi - lo) / lo

    if size < FIB_MIN_WAVE:
        return None

    hi_idx = int(sub["High"].idxmax()) if hasattr(sub["High"].idxmax(), '__int__') else sub["High"].argmax()
    lo_idx = int(sub["Low"].idxmin())  if hasattr(sub["Low"].idxmin(),  '__int__') else sub["Low"].argmin()

    # اتجاه الموجة: هل القمة قبل القاع (هابطة) أم بعده (صاعدة)؟
    if hi_idx < lo_idx:
        # موجة هابطة: من القمة للقاع
        direction  = -1
        wave_start = hi
        wave_end   = lo
    else:
        # موجة صاعدة: من القاع للقمة
        direction  = 1
        wave_start = lo
        wave_end   = hi
...
def _calc_retracement(wave: dict) -> dict:
    """يحسب مستويات التصحيح"""
    hi, lo = wave["wave_hi"], wave["wave_lo"]
    d = hi - lo
    levels = {}
    for name, ratio in FIB_RETRACE_LEVELS.items():
        if wave["direction"] == 1:
            levels[name] = round(hi - d * ratio, 2)
        else:
            levels[name] = round(lo + d * ratio, 2)
    return levels


def _calc_extension(wave: dict) -> dict:
    """يحسب مستويات الامتداد"""
    hi, lo = wave["wave_hi"], wave["wave_lo"]
    d = hi - lo
    levels = {}
    for name, ratio in FIB_EXTEND_LEVELS.items():
        if wave["direction"] == 1:
            levels[name] = round(hi + d * (ratio - 1.0), 2)
        else:
            levels[name] = round(lo - d * (ratio - 1.0), 2)
    return levels


def _calc_pivot_fib(df: pd.DataFrame) -> dict:
    """
    Pivot Point مدمج مع فيبوناتشي
    يستخدم آخر شمعة يومية كاملة
    """
    if len(df) < 2:
        return {}
    prev = df.iloc[-2]
    ...
    return {
        "P":    round(P, 2),
        "R1":   round(P + d * 0.382, 2),
        "R2":   round(P + d * 0.618, 2),
        "R3":   round(P + d * 1.000, 2),
        "S1":   round(P - d * 0.382, 2),
        "S2":   round(P - d * 0.618, 2),
        "S3":   round(P - d * 1.000, 2),
    }


def _golden_pocket(wave: dict) -> dict:
    """منطقة الجيب الذهبي: 61.8% — 65.0% (أقوى منطقة دخول)"""
    hi, lo = wave["wave_hi"], wave["wave_lo"]
    d = hi - lo
    if wave["direction"] == 1:
        return {
            "top":    round(hi - d * FIB_GOLDEN_LO, 2),
            "bottom": round(hi - d * FIB_GOLDEN_HI, 2),
        }
    else:
        return {
            "bottom": round(lo + d * FIB_GOLDEN_LO, 2),
            "top":    round(lo + d * FIB_GOLDEN_HI, 2),
        }


def _find_confluence(price: float, retrace: dict, extend: dict,
                     pivot: dict, buf: float) -> list:
    """
    يبحث عن تقاطع مستويات متعددة قرب السعر الحالي
    يُرجع: قائمة بالمستويات المتقاطعة
    """
    all_levels = {}
    for k, v in retrace.items(): all_levels[f"R{k}"] = v
    for k, v in extend.items():  all_levels[f"E{k}"] = v
    for k, v in pivot.items():   all_levels[f"P-{k}"] = v

    near = []
    for name, level in all_levels.items():
        if abs(price - level) / price <= buf:
            near.append({"name": name, "level": level,
                         "dist_pct": round(abs(price - level) / price * 100, 3)})
    near.sort(key=lambda x: x["dist_pct"])
    return near


def _price_at_fib(price: float, retrace: dict, buf: float) -> Optional[tuple]:
    """يحدد هل السعر عند مستوى فيبوناتشي محدد"""
    for name, level in retrace.items():
        if abs(price - level) / price <= buf:
            return name, level
    return None


def _fib_entry_sl_tp(wave: dict, retrace: dict, extend: dict,
                     price: float, at_val: float) -> Optional[dict]:
    """
    يحسب نقطة الدخول المثلى عند مستوى فيبوناتشي
    مع SL خلف المستوى التالي وTP عند الامتداد
    """
    fib_at = _price_at_fib(price, retrace, FIB_TOUCH_BUF * 2)
    if not fib_at:
        return None

    fib_name, fib_level = fib_at
    direction = wave["direction"]

    if direction == 1:  # شراء عند تصحيح
        entry = fib_level
        # SL خلف مستوى 78.6% أو الموجة
        sl    = max(wave["wave_lo"], fib_level - at_val * 1.5)
        # TP عند امتداد 127.2% و161.8%
        tp1   = extend.get("127.2%", fib_level + at_val * 2)
        tp2   = extend.get("161.8%", fib_level + at_val * 4)
        tp3   = extend.get("261.8%", fib_level + at_val * 7)
    else:  # بيع عند تصحيح
        entry = fib_level
        sl    = min(wave["wave_hi"], fib_level + at_val * 1.5)
        tp1   = extend.get("127.2%", fib_level - at_val * 2)
        tp2   = extend.get("161.8%", fib_level - at_val * 4)
        tp3   = extend.get("261.8%", fib_level - at_val * 7)

    risk = abs(entry - sl)
    rr1  = abs(tp1 - entry) / (risk + 1e-9)
    if rr1 < 1.2:
        return None
    ...
    return {
        ...
        "tp2":      round(tp2, 2),
        "tp3":      round(tp3, 2),
        "rr":       round(abs(tp2 - entry) / (risk + 1e-9), 2),
        "fib_name": fib_name,
        "fib_level": fib_level,
    }


def _fib_signal_score(wave: dict, price: float, retrace: dict,
                      extend: dict, pivot: dict, gp: dict) -> tuple:
    """
    يحسب قوة إشارة فيبوناتشي ويُرجع (score, vote, reasons)
    """
    score   = 0
    vote    = 0
    reasons = []
    ...
    # ── 1. هل السعر في الجيب الذهبي؟ (أقوى إشارة) ──
    in_gp = (min(gp["bottom"], gp["top"]) * (1-buf) <=
             price <= max(gp["bottom"], gp["top"]) * (1+buf))
    if in_gp:
        score += 35
        vote   = d
        reasons.append(f"🏆 الجيب الذهبي (61.8%–65%) عند {gp.get('top' if d==1 else 'bottom',0):.2f}  — أقوى منطقة دخول")

    # ── 2. هل السعر عند مستوى تصحيح مهم؟ ──
    fib_at = _price_at_fib(price, retrace, buf)
    if fib_at:
        fname, flevel = fib_at
        weight = {"61.8%": 25, "50.0%": 20, "38.2%": 15,
                  "65.0%": 22, "78.6%": 12, "23.6%": 8}.get(fname, 10)
        score += weight
        vote   = d
        reasons.append(f"📐 السعر عند مستوى {fname} = {flevel:.2f}")

    # ── 3. تقاطع مستويات (Confluence) ──
    confl = _find_confluence(price, retrace, extend, pivot, buf * 2)
    if len(confl) >= 2:
        score += 15 * min(len(confl), 3)
        reasons.append(f"🔀 تقاطع {len(confl)} مستويات عند {price:.2f}")
        for c in confl[:3]:
            reasons.append(f"   ↳ {c['name']} = {c['level']:.2f} (±{c['dist_pct']:.2f}%)")

    # ── 4. السعر بين الـ Pivot R2 وR3 (مقاومة) ──
    if pivot and vote == 0:
        if pivot.get("R2") and pivot.get("R3"):
            if pivot["R2"] <= price <= pivot["R3"]:
                score += 12; vote = -1
                reasons.append(f"📊 Pivot: بين R2={pivot['R2']} وR3={pivot['R3']} — ضغط بيع")
        if pivot.get("S2") and pivot.get("S1"):
            if pivot["S2"] <= price <= pivot["S1"]:
                score += 12; vote = 1
                reasons.append(f"📊 Pivot: بين S1={pivot['S1']} وS2={pivot['S2']} — دعم شراء")

    # ── 5. قرب هدف امتداد محقق ──
    for ename, elevel in extend.items():
        if abs(price - elevel) / price <= buf:
            score += 8
            reasons.append(f"🎯 عند هدف امتداد {ename} = {elevel:.2f}")
            break

    return score, vote, reasons


def analyze_fibonacci() -> dict:
    """
    الوحدة الرئيسية لاستراتيجيات فيبوناتشي الشاملة.
    تحلّل على 3 إطارات زمنية وتُرجع إشارة مدمجة.
    """
    _empty = {
        "vote": 0, "score": 0, "conf": 0, "signal_type": None,
        "wave": None, "retrace": {}, "extend": {}, "pivot": {}, "gp": {},
        "confluence": [], "entry": None, "sl": None,
        "tp1": None, "tp2": None, "tp3": None, "rr": None,
        "reasons": [], "levels_1h": {}, "levels_4h": {},
    }
    ...
    price  = float(df1h["Close"].iloc[-1])
    at_val = float(atr_fn(df1h).iloc[-1])

    # ── تحليل على 1h ──
    wave1h = _detect_wave(df1h)
    if wave1h is None:
        return {**_empty, "reasons": ["⚠️ لا توجد موجة واضحة على 1h"]}

    retrace1h = _calc_retracement(wave1h)
    extend1h  = _calc_extension(wave1h)
    pivot1h   = _calc_pivot_fib(df1h)
    gp1h      = _golden_pocket(wave1h)

    score1h, vote1h, reasons1h = _fib_signal_score(
        wave1h, price, retrace1h, extend1h, pivot1h, gp1h)

    # ── تحليل على 4h للتأكيد ──
    score4h = 0; vote4h = 0
    wave4h = retrace4h = extend4h = pivot4h = gp4h = None
    levels_4h = {}
    if df4h is not None:
        wave4h    = _detect_wave(df4h)
        if wave4h:
            retrace4h = _calc_retracement(wave4h)
            extend4h  = _calc_extension(wave4h)
            pivot4h   = _calc_pivot_fib(df4h)
            gp4h      = _golden_pocket(wave4h)
            score4h, vote4h, _ = _fib_signal_score(
                wave4h, price, retrace4h, extend4h, pivot4h, gp4h)
            levels_4h = {**retrace4h, **extend4h}
            if vote4h == vote1h and vote1h != 0:
                score1h += 20
                reasons1h.append("✅ تأكيد على 4h — توافق الإطارين")

    # ── تحليل على 15m للتوقيت ──
    if df15 is not None:
        wave15 = _detect_wave(df15)
        if wave15 and wave15["direction"] == vote1h:
            score1h += 10
            reasons1h.append("✅ 15m يدعم الاتجاه")

    # ── حساب نقاط الدخول ──
    levels_entry = _fib_entry_sl_tp(wave1h, retrace1h, extend1h, price, at_val)

    # ── التحقق من الحد الأدنى ──
    min_score = 30
    if score1h < min_score or vote1h == 0:
        near_levels = _find_confluence(price, retrace1h, extend1h, pivot1h, 0.008)
        reasons1h.append(f"⏳ لا إشارة — أقرب مستوى: {near_levels[0]['name']}={near_levels[0]['level']:.2f}" if near_levels else "⏳ لا إشارة فيبوناتشي حالياً")
        return {**_empty,
                "wave": wave1h, "retrace": retrace1h, "extend": extend1h,
                "pivot": pivot1h, "gp": gp1h, "reasons": reasons1h,
                "score": score1h, "levels_1h": {**retrace1h, **extend1h},
                "levels_4h": levels_4h}

    conf = min(95, int(score1h * 1.2))
    entry = sl = tp1 = tp2 = tp3 = rr = None
    signal_type = "تصحيح فيبوناتشي"

    if levels_entry:
        entry = levels_entry["entry"]
        sl    = levels_entry["sl"]
        tp1   = levels_entry["tp1"]
        tp2   = levels_entry["tp2"]
        tp3   = levels_entry["tp3"]
        rr    = levels_entry["rr"]
        signal_type = f"دخول عند {levels_entry['fib_name']}"
        reasons1h.append(f"🎯 دخول: {entry:.2f} | SL: {sl:.2f} | TP2: {tp2:.2f} | R:R={rr:.1f}")
    else:
        # استخدم ATR إذا لم يكن السعر عند مستوى محدد
        d = wave1h["direction"]
        entry = price
        sl    = price - at_val * 1.5 if d == 1 else price + at_val * 1.5
        tp1   = extend1h.get("127.2%", price + at_val * 2 if d == 1 else price - at_val * 2)
        tp2   = extend1h.get("161.8%", price + at_val * 4 if d == 1 else price - at_val * 4)
        tp3   = extend1h.get("261.8%", price + at_val * 7 if d == 1 else price - at_val * 7)
        rr    = round(abs(tp2 - entry) / (abs(sl - entry) + 1e-9), 2)
        if rr < 1.5:
            vote1h = 0

    conf_zones = _find_confluence(price, retrace1h, extend1h, pivot1h, FIB_TOUCH_BUF * 3)

    log.info(f"[Fib] vote={vote1h} score={score1h} wave={wave1h['size_pct']:.2f}% direction={'↑' if wave1h['direction']==1 else '↓'}")
    ...
    return {
        ...
        "pivot":       pivot1h,
        "gp":          gp1h,
        "confluence":  conf_zones,
        "entry":       round(entry, 2) if entry and vote1h else None,
        "sl":          round(sl, 2)    if sl    and vote1h else None,
        "tp1":         round(tp1, 2)   if tp1   and vote1h else None,
        "tp2":         round(tp2, 2)   if tp2   and vote1h else None,
        "tp3":         round(tp3, 2)   if tp3   and vote1h else None,
        "rr":          rr              if vote1h else None,
        "reasons":     reasons1h,
        "levels_1h":   {**retrace1h, **extend1h},
        "levels_4h":   levels_4h,
    }


# ══════════════════════════════════════════════════════════════════
# MODULE 7 — Reversal Patterns (أنماط الانعكاس الكاملة)
# ══════════════════════════════════════════════════════════════════
#
# الأنماط المدعومة:
# ─── شمعيات ────────────────────────────────────────────────────
#   ① Hammer / Hanging Man          ← مطرقة / رجل مشنوق
#   ② Inverted Hammer / Shooting Star ← مطرقة معكوسة / نجمة راحلة
#   ③ Bullish/Bearish Engulfing      ← الابتلاع الصاعد/الهابط
#   ④ Doji (Standard/Dragonfly/Gravestone) ← دوجي
#   ⑤ Morning Star / Evening Star    ← نجمة الصباح/المساء
#   ⑥ Piercing Line / Dark Cloud     ← خط الاختراق / سحابة داكنة
#   ⑦ Three White Soldiers / Three Black Crows ← ثلاثة جنود/غربان
#   ⑧ Tweezer Top / Bottom           ← الملقط
# ─── مخططية ───────────────────────────────────────────────────
#   ⑨  Double Top / Double Bottom    ← قمة مزدوجة / قاع مزدوج
#   ⑩  Head & Shoulders / Inv H&S    ← رأس وكتفين
#   ⑪  Rising/Falling Wedge          ← إسفين صاعد/هابط

RP_SHADOW_RATIO  = 2.0   # نسبة الظل للجسم في المطرقة
RP_SMALL_BODY    = 0.25  # نسبة الجسم الصغير (دوجي)
RP_ENGULF_RATIO  = 1.05  # نسبة ابتلاع الجسم
RP_MIN_BODY_PCT  = 0.001 # حد أدنى حجم الشمعة 0.1%
RP_HS_LOOKBACK   = 60    # عدد الشموع لكشف رأس وكتفين
RP_DT_TOLERANCE  = 0.015 # تسامح القمة المزدوجة 1.5%
...
def _candle_parts(c) -> dict:
    ...
    return {"o":o,"h":h,"l":l,"c":cl,"body":body,"full":full,"bull":bull,
            "upper_s":upper_s,"lower_s":lower_s,"mid":mid,"body_pct":body_pct}
...
def _hammer(p: dict) -> Optional[tuple]:
    """مطرقة (صاعدة) / رجل مشنوق (هابطة)"""
    if p["body"] < p["full"] * RP_MIN_BODY_PCT: return None
    if p["lower_s"] < p["body"] * RP_SHADOW_RATIO: return None
    if p["upper_s"] > p["body"] * 0.5: return None
    if p["bull"]:
        return 1, "🔨 مطرقة (Hammer) — انعكاس صاعد محتمل"
    else:
        return -1, "🪝 رجل مشنوق (Hanging Man) — انعكاس هابط محتمل"


def _inverted_hammer(p: dict) -> Optional[tuple]:
    """مطرقة معكوسة (صاعدة) / نجمة راحلة (هابطة)"""
    if p["body"] < p["full"] * RP_MIN_BODY_PCT: return None
    if p["upper_s"] < p["body"] * RP_SHADOW_RATIO: return None
    if p["lower_s"] > p["body"] * 0.5: return None
    if p["bull"]:
        return 1, "🔄 مطرقة معكوسة (Inv. Hammer) — انعكاس صاعد"
    else:
        return -1, "⭐ نجمة راحلة (Shooting Star) — انعكاس هابط"


def _doji(p: dict) -> Optional[tuple]:
    """دوجي — تردد السوق"""
    if p["body_pct"] > RP_SMALL_BODY: return None
    if p["lower_s"] > p["upper_s"] * 3:
        return 1, "🐉 دوجي اليعسوب (Dragonfly Doji) — دعم قوي"
    elif p["upper_s"] > p["lower_s"] * 3:
        return -1, "🪦 دوجي شاهد القبر (Gravestone Doji) — مقاومة قوية"
    else:
        return 0, "⚖️ دوجي — تردد وعدم حسم"


def _engulfing(prev: dict, curr: dict) -> Optional[tuple]:
    """الابتلاع الصاعد والهابط"""
    if curr["body"] < prev["body"] * RP_ENGULF_RATIO: return None
    if curr["bull"] and not prev["bull"]:
        if curr["o"] <= prev["c"] and curr["c"] >= prev["o"]:
            return 1, "🟢 ابتلاع صاعد (Bullish Engulfing) — ⭐⭐⭐⭐"
    if not curr["bull"] and prev["bull"]:
        if curr["o"] >= prev["c"] and curr["c"] <= prev["o"]:
            return -1, "🔴 ابتلاع هابط (Bearish Engulfing) — ⭐⭐⭐⭐"
    return None


def _piercing_dark(prev: dict, curr: dict) -> Optional[tuple]:
    """خط الاختراق / السحابة الداكنة"""
    mid_prev = (prev["o"] + prev["c"]) / 2
    if not prev["bull"] and curr["bull"]:
        if curr["o"] < prev["c"] and curr["c"] > mid_prev:
            return 1, "💡 خط الاختراق (Piercing Line) — ⭐⭐⭐"
    if prev["bull"] and not curr["bull"]:
        if curr["o"] > prev["c"] and curr["c"] < mid_prev:
            return -1, "☁️ السحابة الداكنة (Dark Cloud Cover) — ⭐⭐⭐"
    return None


def _morning_evening_star(c1: dict, c2: dict, c3: dict) -> Optional[tuple]:
    """نجمة الصباح / نجمة المساء"""
    if c2["body_pct"] > RP_SMALL_BODY * 1.5: return None
    # نجمة الصباح
    if (not c1["bull"] and c1["body"] > c1["full"] * 0.4 and
            c3["bull"] and c3["body"] > c3["full"] * 0.4 and
            c3["c"] > (c1["o"] + c1["c"]) / 2):
        return 1, "🌅 نجمة الصباح (Morning Star) — ⭐⭐⭐⭐⭐ قوي جداً"
    # نجمة المساء
    if (c1["bull"] and c1["body"] > c1["full"] * 0.4 and
            not c3["bull"] and c3["body"] > c3["full"] * 0.4 and
            c3["c"] < (c1["o"] + c1["c"]) / 2):
        return -1, "🌆 نجمة المساء (Evening Star) — ⭐⭐⭐⭐⭐ قوي جداً"
    return None


def _three_soldiers_crows(candles: list) -> Optional[tuple]:
    """ثلاثة جنود بيض / ثلاثة غربان سود"""
    if len(candles) < 3: return None
    c1, c2, c3 = candles[-3], candles[-2], candles[-1]
    # ثلاثة جنود
    if all(c["bull"] for c in [c1, c2, c3]):
        if c2["c"] > c1["c"] and c3["c"] > c2["c"]:
            if all(c["body"] > c["full"] * 0.5 for c in [c1, c2, c3]):
                return 1, "💂 ثلاثة جنود بيض (3 White Soldiers) — ⭐⭐⭐⭐⭐"
    # ثلاثة غربان
    if all(not c["bull"] for c in [c1, c2, c3]):
        if c2["c"] < c1["c"] and c3["c"] < c2["c"]:
            if all(c["body"] > c["full"] * 0.5 for c in [c1, c2, c3]):
                return -1, "🐦‍⬛ ثلاثة غربان سود (3 Black Crows) — ⭐⭐⭐⭐⭐"
    return None


def _tweezer(prev: dict, curr: dict) -> Optional[tuple]:
    """الملقط العلوي / السفلي"""
    tol = (prev["h"] - prev["l"]) * 0.02
    if abs(prev["h"] - curr["h"]) <= tol and prev["bull"] and not curr["bull"]:
        return -1, "🔧 ملقط علوي (Tweezer Top) — ⭐⭐⭐"
    if abs(prev["l"] - curr["l"]) <= tol and not prev["bull"] and curr["bull"]:
        return 1, "🔧 ملقط سفلي (Tweezer Bottom) — ⭐⭐⭐"
    return None
...
def _double_top_bottom(df: pd.DataFrame, price: float) -> Optional[tuple]:
    """قمة مزدوجة / قاع مزدوج"""
    sub  = df.tail(40)
    hi   = sub["High"].values
    lo   = sub["Low"].values
    n    = len(sub)
    tol  = price * RP_DT_TOLERANCE

    # ابحث عن قمتين متقاربتين
    peaks = [i for i in range(2, n-2)
             if hi[i] > hi[i-1] and hi[i] > hi[i-2]
             and hi[i] > hi[i+1] and hi[i] > hi[i+2]]
    for i in range(len(peaks)-1):
        p1, p2 = peaks[i], peaks[i+1]
        if abs(hi[p1] - hi[p2]) <= tol and p2 - p1 >= 5:
            neckline = min(lo[p1:p2+1])
            if price < neckline * (1 + tol):
                return -1, (f"🏔️ قمة مزدوجة (Double Top) | القمتان: {hi[p1]:.2f} & {hi[p2]:.2f}"
                            f" | Neckline: {neckline:.2f}")

    # ابحث عن قاعين متقاربين
    troughs = [i for i in range(2, n-2)
               if lo[i] < lo[i-1] and lo[i] < lo[i-2]
               and lo[i] < lo[i+1] and lo[i] < lo[i+2]]
    for i in range(len(troughs)-1):
        t1, t2 = troughs[i], troughs[i+1]
        if abs(lo[t1] - lo[t2]) <= tol and t2 - t1 >= 5:
            neckline = max(hi[t1:t2+1])
            if price > neckline * (1 - tol):
                return 1, (f"🏜️ قاع مزدوج (Double Bottom) | القاعان: {lo[t1]:.2f} & {lo[t2]:.2f}"
                           f" | Neckline: {neckline:.2f}")
    return None


def _head_shoulders(df: pd.DataFrame, price: float) -> Optional[tuple]:
    """رأس وكتفين / رأس وكتفين معكوس"""
    sub = df.tail(RP_HS_LOOKBACK)
    hi  = sub["High"].values
    lo  = sub["Low"].values
    n   = len(sub)

    # كشف القمم والقيعان
    peaks = [i for i in range(3, n-3)
             if hi[i] == max(hi[max(0,i-5):i+6])]
    troughs = [i for i in range(3, n-3)
               if lo[i] == min(lo[max(0,i-5):i+6])]

    # رأس وكتفين عادي (3 قمم: كتف + رأس + كتف)
    if len(peaks) >= 3:
        for i in range(len(peaks)-2):
            ls, h, rs = peaks[i], peaks[i+1], peaks[i+2]
            if (hi[h] > hi[ls] * 1.01 and hi[h] > hi[rs] * 1.01 and
                    abs(hi[ls] - hi[rs]) / hi[h] < 0.05):
                between = [t for t in troughs if ls < t < rs]
                if len(between) >= 2:
                    neck = (lo[between[0]] + lo[between[-1]]) / 2
                    if price < neck * 1.005:
                        return -1, (f"👤 رأس وكتفين (H&S) | رأس={hi[h]:.2f}"
                                    f" | كتف أيسر={hi[ls]:.2f} | Neckline={neck:.2f}")

    # رأس وكتفين معكوس (3 قيعان)
    if len(troughs) >= 3:
        for i in range(len(troughs)-2):
            ls, h, rs = troughs[i], troughs[i+1], troughs[i+2]
            if (lo[h] < lo[ls] * 0.99 and lo[h] < lo[rs] * 0.99 and
                    abs(lo[ls] - lo[rs]) / lo[h] < 0.05):
                between = [p for p in peaks if ls < p < rs]
                if len(between) >= 2:
                    neck = (hi[between[0]] + hi[between[-1]]) / 2
                    if price > neck * 0.995:
                        return 1, (f"🙃 رأس وكتفين معكوس (Inv H&S) | قاع={lo[h]:.2f}"
                                   f" | Neckline={neck:.2f}")
    return None


def _wedge(df: pd.DataFrame) -> Optional[tuple]:
    """إسفين صاعد (هابط) / إسفين هابط (صاعد)"""
    sub = df.tail(30)
    n   = len(sub)
    if n < 10: return None
    ...
    # خط اتجاه القمم
    import numpy as _np
    hi_slope = _np.polyfit(x, highs, 1)[0]
    lo_slope = _np.polyfit(x, lows,  1)[0]

    price = float(sub["Close"].iloc[-1])
    atr   = float(atr_fn(sub).iloc[-1])
    tol   = atr * 0.5

    # إسفين صاعد: كلا الخطين صاعدان لكن الأعلى أبطأ من الأسفل
    if hi_slope > 0 and lo_slope > 0 and lo_slope > hi_slope * 1.3:
        if price < float(sub["Low"].iloc[-3]) - tol:
            return -1, f"📐 إسفين صاعد (Rising Wedge) — كسر هابط ⭐⭐⭐⭐"

    # إسفين هابط: كلا الخطين هابطان لكن الأدنى أسرع
    if hi_slope < 0 and lo_slope < 0 and hi_slope > lo_slope * 1.3:
        if price > float(sub["High"].iloc[-3]) + tol:
            return 1, f"📐 إسفين هابط (Falling Wedge) — كسر صاعد ⭐⭐⭐⭐"
...
def analyze_reversal() -> dict:
    """
    يحلّل أنماط الانعكاس على إطار 1h و4h و15m.
    يُرجع الأنماط المكتشفة مع درجة القوة.
    """
    _empty = {
        "vote": 0, "score": 0, "conf": 0, "patterns": [],
        "strongest": None, "entry": None, "sl": None,
        "tp1": None, "tp2": None, "rr": None,
        "signal_type": None, "reasons": [],
    }

    df1h = fetch(SYMBOL, "90d",  "1h")
    df4h = fetch(SYMBOL, "360d", "4h")
    df15 = fetch(SYMBOL, "15d",  "15m")
    ...
    def _run_candle_patterns(df, tf_name, weight):
        nonlocal score, vote
        if df is None or len(df) < 5:
            return
        rows = df.tail(5)
        cp   = [_candle_parts(rows.iloc[i]) for i in range(len(rows))]
        n    = len(cp)
        found = []

        # 1 شمعة
        r = _hammer(cp[-1]);          found.append(r) if r else None
        r = _inverted_hammer(cp[-1]); found.append(r) if r else None
        r = _doji(cp[-1]);            found.append(r) if r else None

        # 2 شمعات
        if n >= 2:
            r = _engulfing(cp[-2], cp[-1]);     found.append(r) if r else None
            r = _piercing_dark(cp[-2], cp[-1]); found.append(r) if r else None
            r = _tweezer(cp[-2], cp[-1]);       found.append(r) if r else None

        # 3 شمعات
        if n >= 3:
            r = _morning_evening_star(cp[-3], cp[-2], cp[-1])
            found.append(r) if r else None
            r = _three_soldiers_crows(cp)
            found.append(r) if r else None

        for sig, name in found:
            w = {"⭐⭐⭐⭐⭐": 30, "⭐⭐⭐⭐": 20, "⭐⭐⭐": 12}.get(
                name[-10:].strip()[-7:] if len(name) > 7 else "⭐", 8)
            patterns.append({"tf": tf_name, "vote": sig, "name": name,
                             "weight": w * weight, "price": price})
            if sig != 0:
                score += w * weight
                if vote == 0: vote = sig
                elif sig == vote: score += 5  # تأكيد من إطار آخر
            reasons.append(f"[{tf_name}] {name}")

    # ── تشغيل الأنماط الشمعية ──
    _run_candle_patterns(df15,  "15m", 0.7)
    _run_candle_patterns(df1h,  "1h",  1.0)
    _run_candle_patterns(df4h,  "4h",  1.4)

    # ── أنماط مخططية على 1h ──
    r = _double_top_bottom(df1h, price)
    if r:
        sig, name = r
        score += 25; vote = sig if vote == 0 else (vote if vote == sig else vote)
        patterns.append({"tf": "1h", "vote": sig, "name": name, "weight": 25, "price": price})
        reasons.append(f"[1h] {name}")

    r = _head_shoulders(df1h, price)
    if r:
        sig, name = r
        score += 30; vote = sig if vote == 0 else vote
        patterns.append({"tf": "1h", "vote": sig, "name": name, "weight": 30, "price": price})
        reasons.append(f"[1h] {name}")

    r = _wedge(df1h)
    if r:
        sig, name = r
        score += 20; vote = sig if vote == 0 else vote
        patterns.append({"tf": "1h", "vote": sig, "name": name, "weight": 20, "price": price})
        reasons.append(f"[1h] {name}")

    # ── تأكيد على 4h ──
    if df4h is not None:
        r = _double_top_bottom(df4h, price)
        if r:
            sig, name = r
            if sig == vote: score += 15; reasons.append(f"[4h] {name} (تأكيد)")
        r = _head_shoulders(df4h, price)
        if r:
            sig, name = r
            if sig == vote: score += 20; reasons.append(f"[4h] {name} (تأكيد)")

    if not patterns or score < 15:
        return {**_empty, "reasons": ["⏳ لا توجد أنماط انعكاس واضحة حالياً"], "score": score}

    # أقوى نمط
    strongest = max(patterns, key=lambda x: x["weight"])
    conf = min(92, int(score * 1.1))
    ...
    log.info(f"[Reversal] vote={vote} score={score} patterns={len(patterns)} strongest={strongest['name'][:30]}")
    ...
    return {
        ...
        "tp1":         round(tp1, 2)   if vote else None,
        "tp2":         round(tp2, 2)   if vote else None,
        "rr":          rr              if vote else None,
        "signal_type": strongest["name"][:40],
        "reasons":     reasons[:8],
    }


# ══════════════════════════════════════════════════════════════════
# SIGNAL QUALITY & RISK MANAGEMENT SYSTEM
# نظام جودة الإشارة وإدارة المخاطر
# ══════════════════════════════════════════════════════════════════
#
# المكونات:
#   ① Signal Quality Score  — تقييم جودة الإشارة (0-100)
#   ② Counter-Move Defense  — خطة الدفاع عند الارتداد المعاكس
#   ③ Dynamic SL/TP         — أهداف ديناميكية بحسب التقلب
#   ④ Position Sizing        — حجم الصفقة المناسب
#   ⑤ Invalidation Levels    — مستويات إلغاء الإشارة
#   ⑥ Re-entry Plan         — خطة الدخول مجدداً بعد الارتداد

SQ_MIN_QUALITY    = 55   # أدنى جودة لإرسال الإشارة
SQ_HIGH_QUALITY   = 75   # جودة عالية — دخول كامل
SQ_LOW_QUALITY    = 55   # جودة متوسطة — دخول نصفي
COUNTER_ATR_MULT  = 0.8  # كم ATR يُعدّ ارتداداً معاكساً
BREAKEVEN_PCT     = 0.4  # متى يُنقل SL للتعادل (40% من TP1)
PARTIAL_TP1_PCT   = 50   # نسبة الإغلاق عند TP1
TRAIL_MULT        = 1.5  # مضاعف الـ Trailing Stop


def calc_signal_quality(agg: dict, v7: dict, smc: dict, ob,
                        fib: dict, rev: dict, rng: dict) -> dict:
    """
    يحسب درجة جودة الإشارة الشاملة (0-100)
    ويُحدد حجم الدخول الموصى به
    """
    score   = 0
    bonuses = []
    maluses = []
    ...
    # ── 1. توافق الاستراتيجيات (أهم عامل) ──
    agree = agg.get("agree", 0)
    total_strats = len([v for v in agg.get("votes",{}).values() if v != 0])
    agree_pct = agree / max(total_strats, 1) * 100
    score += int(agree_pct * 0.30)  # ≤30 نقطة
    bonuses.append(f"✅ توافق {agree}/{total_strats} استراتيجيات (+{int(agree_pct*0.30)})")

    # ── 2. نسبة R:R ──
    rr = agg.get("rr") or 0
    if rr >= 3.0:   score += 20; bonuses.append(f"✅ R:R ممتاز {rr:.1f} (+20)")
    elif rr >= 2.0: score += 15; bonuses.append(f"✅ R:R جيد {rr:.1f} (+15)")
    elif rr >= 1.5: score += 10; bonuses.append(f"⚠️ R:R مقبول {rr:.1f} (+10)")
    else:           maluses.append(f"❌ R:R ضعيف {rr:.1f} (-5)"); score -= 5

    # ── 3. DXY (للذهب) ──
    dxy_txt = v7.get("dxy_txt", "")
    if d == 1 and "ضعف" in dxy_txt:
        score += 8; bonuses.append("✅ DXY ضعيف يدعم الشراء (+8)")
    elif d == -1 and "قوة" in dxy_txt:
        score += 8; bonuses.append("✅ DXY قوي يدعم البيع (+8)")
    elif (d == 1 and "قوة" in dxy_txt) or (d == -1 and "ضعف" in dxy_txt):
        score -= 8; maluses.append("⚠️ DXY يعاكس الاتجاه (-8)")

    # ── 4. حجم التداول ──
    if v7.get("vol_ok"):
        score += 7; bonuses.append("✅ حجم مرتفع (+7)")
    else:
        score -= 5; maluses.append("⚠️ حجم منخفض (-5)")

    # ── 5. تأكيد فيبوناتشي ──
    if fib and fib.get("vote") == d:
        fib_score = fib.get("score", 0)
        pts = min(12, int(fib_score * 0.15))
        score += pts; bonuses.append(f"✅ فيبوناتشي يدعم (+{pts})")
        if fib.get("gp") and fib.get("vote") == d:
            score += 5; bonuses.append("🏆 الجيب الذهبي (+5)")

    # ── 6. أنماط الانعكاس ──
    if rev and rev.get("vote") == d:
        rev_score = rev.get("score", 0)
        pts = min(10, int(rev_score * 0.1))
        score += pts; bonuses.append(f"✅ أنماط انعكاس تدعم (+{pts})")
        strongest = (rev.get("strongest") or {}).get("name", "")
        if "نجمة الصباح" in strongest or "نجمة المساء" in strongest:
            score += 5; bonuses.append("⭐ نجمة قوية (+5)")
        if "رأس وكتفين" in strongest:
            score += 8; bonuses.append("👤 رأس وكتفين (+8)")

    # ── 7. Divergence RSI/MACD ──
    strats = v7.get("strats", {})
    for nm in ["V7_RSI_Divergence", "V7_MACD_Divergence"]:
        s = strats.get(nm, {})
        if s.get("signal") == d:
            score += 6; bonuses.append(f"✅ {nm.replace('V7_','')} (+6)")

    # ── 8. SMC Score ──
    smc_score = abs(smc.get("score", 0))
    if smc_score >= 6: score += 8; bonuses.append(f"✅ SMC قوي ({smc_score}) (+8)")
    elif smc_score >= 4: score += 4

    # ── 9. جلسة التداول ──
    session = trading_session()
    if "لندن" in session or "نيويورك" in session:
        score += 5; bonuses.append("✅ جلسة رئيسية (+5)")
    elif "آسيا" in session:
        score -= 3; maluses.append("⚠️ جلسة آسيا (-3)")

    # ── 10. عكس الاتجاه (ضبابية) ──
    bear_v = sum(1 for v in agg.get("votes",{}).values() if v == -d)
    if bear_v >= 2:
        score -= 10; maluses.append(f"⚠️ {bear_v} استراتيجيات ضد الاتجاه (-10)")

    score = max(0, min(100, score))

    # ── تحديد حجم الدخول ──
    if score >= SQ_HIGH_QUALITY:
        size_pct = 100; size_lbl = "دخول كامل 💪"
    elif score >= SQ_MIN_QUALITY:
        size_pct = 50;  size_lbl = "دخول نصفي ⚠️"
    else:
        size_pct = 0;   size_lbl = "لا تدخل ❌"

    # ── تصنيف الجودة ──
    if score >= 80:   grade = "A+ ممتاز"
    elif score >= 70: grade = "A  جيد جداً"
    elif score >= 60: grade = "B  جيد"
    elif score >= 50: grade = "C  مقبول"
    else:             grade = "D  ضعيف"
    ...
    return {
        ...
        "bonuses":  bonuses,
        "maluses":  maluses,
        "tradeable": score >= SQ_MIN_QUALITY,
    }


def calc_counter_move_plan(agg: dict, price: float, at_val: float) -> dict:
    """
    يبني خطة دفاعية شاملة للتعامل مع الارتداد المعاكس المفاجئ
    يُرجع: مستويات التحذير + الخطوط الدفاعية + خطط البديلة
    """
    d      = agg.get("d", 0)
    entry  = agg.get("entry") or price
    sl     = agg.get("sl")   or (price - at_val * 2 if d == 1 else price + at_val * 2)
    tp1    = agg.get("tp1")  or (price + at_val * 2 if d == 1 else price - at_val * 2)
    tp2    = agg.get("tp2")  or (price + at_val * 4 if d == 1 else price - at_val * 4)
    risk   = abs(entry - sl)

    if d == 1:  # شراء
        # ① تحذير مبكر: الارتداد 40% من المسافة للـ SL
        warn_1    = round(entry - risk * 0.40, 2)
        # ② تعادل: نقل SL للدخول عند تحقيق 40% من TP1
        breakeven_at  = round(entry + (tp1 - entry) * BREAKEVEN_PCT, 2)
        breakeven_sl  = round(entry + at_val * 0.1, 2)  # SL فوق الدخول قليلاً
        # ③ إغلاق جزئي عند TP1
        partial_close = round(tp1, 2)
        # ④ Trailing Stop بعد TP1
        trail_sl_1    = round(tp1 - at_val * TRAIL_MULT, 2)
        trail_sl_2    = round(tp2 - at_val * TRAIL_MULT, 2)
        # ⑤ مستوى إلغاء الإشارة
        invalidation  = round(sl - at_val * 0.3, 2)
        # ⑥ إعادة الدخول بعد الارتداد
        reentry_zone  = (round(entry - at_val * 0.5, 2), round(entry + at_val * 0.5, 2))
        counter_warn  = f"⚠️ انتبه إذا كسر {warn_1:.2f} — ضع إشعاراً"
        defend_plan   = [
            f"① عند {warn_1:.2f}: راقب الزخم — لا تُضف مركزاً",
            f"② عند {breakeven_at:.2f} ربح: انقل SL للتعادل → {breakeven_sl:.2f}",
            f"③ عند TP1 ({partial_close:.2f}): أغلق 50% واحتفظ بالباقي",
            f"④ Trailing Stop: SL يتبع السعر بفارق {at_val*TRAIL_MULT:.2f}",
        ]
    else:  # بيع
        warn_1    = round(entry + risk * 0.40, 2)
        breakeven_at  = round(entry - (entry - tp1) * BREAKEVEN_PCT, 2)
        breakeven_sl  = round(entry - at_val * 0.1, 2)
        partial_close = round(tp1, 2)
        trail_sl_1    = round(tp1 + at_val * TRAIL_MULT, 2)
        trail_sl_2    = round(tp2 + at_val * TRAIL_MULT, 2)
        invalidation  = round(sl + at_val * 0.3, 2)
        reentry_zone  = (round(entry - at_val * 0.5, 2), round(entry + at_val * 0.5, 2))
        counter_warn  = f"⚠️ انتبه إذا كسر {warn_1:.2f} فوقاً — ضع إشعاراً"
        defend_plan   = [
            f"① عند {warn_1:.2f}: راقب الزخم — لا تُضف مركزاً",
            f"② عند {breakeven_at:.2f} ربح: انقل SL للتعادل → {breakeven_sl:.2f}",
            f"③ عند TP1 ({partial_close:.2f}): أغلق 50% واحتفظ بالباقي",
    ...
    # سيناريو 1: ارتداد خفيف (دون مستوى التحذير)
    scenarios.append({
        "type":  "ارتداد خفيف",
        "level": warn_1,
        "color": "🟡",
        "action": "احتفظ بالمركز — الاتجاه سليم — ضع إشعاراً عند " + str(warn_1),
        "prob":  "عالية — طبيعي في أي حركة"
    })

    # سيناريو 2: ارتداد متوسط (وصل لمنطقة الخطر)
    danger_zone = round(entry - risk * 0.70 if d == 1 else entry + risk * 0.70, 2)
    scenarios.append({
        "type":  "ارتداد متوسط",
        "level": danger_zone,
        "color": "🟠",
        "action": f"أغلق 30% وانقل SL للتعادل ({breakeven_sl:.2f})",
        "prob":  "متوسطة — راجع الأسباب"
    })

    # سيناريو 3: كسر الـ SL
    scenarios.append({
        "type":  "كسر Stop Loss",
        "level": sl,
        "color": "🔴",
        "action": "أغلق فوراً — لا تتمسك بالأمل — الخسارة جزء من التداول",
        "prob":  "منخفضة إذا كانت الجودة عالية"
    })

    # سيناريو 4: ارتداد حاد (ما بعد SL)
    scenarios.append({
        "type":  "ارتداد حاد",
        "level": invalidation,
        "color": "🚨",
        "action": f"إشارة ملغاة — انتظر تكوّن نمط جديد في المنطقة {reentry_zone[0]:.2f}–{reentry_zone[1]:.2f}",
        "prob":  "نادرة — عادة حدث أساسي"
    })

    # ── خطة إعادة الدخول ──
    reentry_plan = [
        f"① انتظر تثبيت السعر في منطقة {reentry_zone[0]:.2f}–{reentry_zone[1]:.2f}",
        "② تحقق من ظهور نمط انعكاسي جديد (مطرقة / ابتلاع)",
        "③ تأكد أن الاستراتيجيات تتوافق من جديد (2+ توافق)",
        "④ أدخل بحجم أصغر (50%) مع SL ضيق",
        "⑤ إذا فشل الدخول الثاني — ابتعد عن هذه الفرصة",
    ]

    # ── تحذيرات الأحداث الأساسية ──
    fundamental_warnings = [
        "📰 تحقق من التقويم الاقتصادي قبل الدخول",
        "🏦 بيانات التضخم (CPI) تُسبب تقلبات حادة",
        "💬 خطابات الفيدرالي تلغي التحليل التقني أحياناً",
        "🌍 الأحداث الجيوسياسية تُربك أنماط الذهب",
    ]
    ...
    return {
        ...
        "trail_sl_1":     trail_sl_1,
        "trail_sl_2":     trail_sl_2,
        "invalidation":   invalidation,
        "reentry_zone":   reentry_zone,
        "counter_warn":   counter_warn,
        "defend_plan":    defend_plan,
        "scenarios":      scenarios,
...
def _quality_badge(score: int) -> str:
    ...
    if score >= 50: return "⚠️ C"
    return "❌ D"


def msg_quality_section(sq: dict, cm: dict, d: int) -> str:
    """
    يبني قسم جودة الإشارة وخطة إدارة المخاطر
    لإضافته في رسالة الإشارة
    """
    lines = []
    score = sq["score"]
    ...
    lines.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    lines.append(f"🎖️ <b>جودة الإشارة: {badge} {score}/100</b>")
    lines.append(f"   📊 {sq['grade']}  |  💰 {sq['size_lbl']}")

    # عوامل إيجابية (أهم 3)
    if sq["bonuses"]:
        for b in sq["bonuses"][:3]:
            lines.append(f"   {b}")

    # عوامل سلبية
    if sq["maluses"]:
        for m in sq["maluses"][:2]:
            lines.append(f"   {m}")

    lines.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    lines.append("🛡️ <b>خطة إدارة المخاطر:</b>")
    for step in cm["defend_plan"]:
        lines.append(f"   {esc(step)}")

    lines.append("")
    lines.append("⚠️ <b>سيناريوهات الارتداد المعاكس:</b>")
    for sc in cm["scenarios"]:
        lines.append(
            f"   {sc['color']} <b>{esc(sc['type'])}</b> عند {sc['level']:.2f}"
            f"\n      → {esc(sc['action'])}"
        )

    lines.append("")
    lines.append(f"🔄 <b>منطقة الدخول بعد الارتداد:</b>")
    lines.append(
        f"   {cm['reentry_zone'][0]:.2f} — {cm['reentry_zone'][1]:.2f}"
    )
    lines.append(f"   مستوى الإلغاء: <b>{cm['invalidation']:.2f}</b>")

    return "\n".join(lines)


# ══════════════════════════════════════════════════════════════════
# MODULE 8 — Single Candle Strategy (استراتيجية الشمعة الواحدة)
# ══════════════════════════════════════════════════════════════════
#
# المفهوم: الشمعة الواحدة تحكي قصة كاملة — القوة، الضعف، التردد
# نحلل آخر شمعة مكتملة على 4 إطارات زمنية بعمق شامل
#
# الأنواع المحللة:
#   ① Marubozu        — شمعة قوية بلا ظلال (أقوى إشارة)
#   ② Spinning Top    — جسم صغير + ظلال طويلة (تردد)
#   ③ Hammer / Pin Bar — ظل سفلي طويل (دعم قوي)
#   ④ Inverted Hammer  — ظل علوي طويل
#   ⑤ Doji types       — توازن كامل
#   ⑥ Strong Bullish/Bearish — شمعة اتجاهية قوية
#   ⑦ High Wave        — ظلال طويلة من الجانبين
#   ⑧ Belt Hold        — افتتاح عند القمة/القاع
#   ⑨ Kicker           — فجوة انعكاسية حادة
#   ⑩ Candle Range     — موقع الإغلاق ضمن المدى

SC_MARUBOZU_THRESH  = 0.95   # نسبة الجسم من الكل لتكون Marubozu
SC_STRONG_BODY      = 0.70   # شمعة قوية
SC_SMALL_BODY       = 0.25   # جسم صغير
SC_DOJI_THRESH      = 0.08   # دوجي: جسم أقل من 8% من المدى
SC_LONG_SHADOW      = 2.5    # ظل طويل: أكثر من 2.5× الجسم
SC_MIN_CANDLE       = 0.003  # أدنى مدى للشمعة (تجنب الشموع الميتة)
SC_CONTEXT_LOOKBACK = 20     # عدد الشموع للسياق
SC_ATR_PERIODS      = 14     # ATR للمقارنة
SC_VOL_LOOKBACK     = 20     # متوسط الحجم


def _analyze_single_candle(c, prev_candles, at_val: float, avg_vol: float) -> dict:
    """
    يحلل شمعة واحدة بعمق كامل
    c: آخر شمعة | prev_candles: شموع سابقة
    at_val: قيمة ATR | avg_vol: متوسط الحجم
    """
    o  = float(c.get("Open",  c.get("open",  0)))
    h  = float(c.get("High",  c.get("high",  0)))
    l  = float(c.get("Low",   c.get("low",   0)))
    cl = float(c.get("Close", c.get("close", 0)))
    vol= float(c.get("Volume",c.get("volume",0)))
    ...
    lower_s = min(cl, o) - l
    body_pct = body / full  if full > 0 else 0
    upper_pct= upper_s/full if full > 0 else 0
    lower_pct= lower_s/full if full > 0 else 0
    # موقع الإغلاق ضمن مدى الشمعة (0=قاع, 1=قمة)
    close_pos= (cl - l) / full if full > 0 else 0.5
    # قوة الشمعة مقارنة بـ ATR
    size_vs_atr = full / at_val if at_val > 0 else 1
    # قوة الحجم
    vol_ratio   = vol / avg_vol if avg_vol > 0 else 1

    # ── تجنب الشموع الميتة ──
    if full < cl * SC_MIN_CANDLE:
        return {"type":"dead","vote":0,"strength":0,"reason":"شمعة بلا حركة"}
    ...
    # ① Marubozu — أقوى إشارة
    # ════════════════════════════════════
    if body_pct >= SC_MARUBOZU_THRESH:
        if bull:
            candle_type = "bullish_marubozu"
            vote, strength = 1, 9
            reason = f"🟩 Marubozu صاعد | جسم={body_pct*100:.0f}%"
        else:
            candle_type = "bearish_marubozu"
            vote, strength = -1, 9
            reason = f"🟥 Marubozu هابط | جسم={body_pct*100:.0f}%"

    # ════════════════════════════════════
    # ② Doji — تردد
    # ════════════════════════════════════
    elif body_pct <= SC_DOJI_THRESH:
        if lower_pct > 0.45 and upper_pct < 0.10:
            candle_type = "dragonfly_doji"
            vote, strength = 1, 7
            reason = "🐉 دوجي اليعسوب — دعم قوي جداً"
        elif upper_pct > 0.45 and lower_pct < 0.10:
            candle_type = "gravestone_doji"
            vote, strength = -1, 7
            reason = "🪦 دوجي شاهد القبر — مقاومة قوية"
        else:
            candle_type = "doji"
            vote, strength = 0, 4
            reason = "⚖️ دوجي — تردد في السوق"

    # ════════════════════════════════════
    # ③ Hammer / Pin Bar
    # ════════════════════════════════════
    elif (lower_pct >= 0.55 and upper_pct <= 0.15 and body_pct <= 0.30):
        if bull:
            candle_type = "hammer"
            vote, strength = 1, 8
            reason = f"🔨 مطرقة صاعدة | ظل سفلي={lower_pct*100:.0f}%"
        else:
            candle_type = "hanging_man"
            vote, strength = -1, 6
            reason = f"🪝 رجل مشنوق | ظل سفلي={lower_pct*100:.0f}%"

    # ════════════════════════════════════
    # ④ Inverted Hammer / Shooting Star
    # ════════════════════════════════════
    elif (upper_pct >= 0.55 and lower_pct <= 0.15 and body_pct <= 0.30):
        if bull:
            candle_type = "inverted_hammer"
            vote, strength = 1, 6
            reason = f"🔄 مطرقة معكوسة | ظل علوي={upper_pct*100:.0f}%"
        else:
            candle_type = "shooting_star"
            vote, strength = -1, 8
            reason = f"⭐ نجمة راحلة | ظل علوي={upper_pct*100:.0f}%"

    # ════════════════════════════════════
    # ⑤ Strong Candle — شمعة اتجاهية
    # ════════════════════════════════════
    elif body_pct >= SC_STRONG_BODY:
        if bull:
            candle_type = "strong_bull"
            vote, strength = 1, 7
            reason = f"💚 شمعة صاعدة قوية | جسم={body_pct*100:.0f}%"
        else:
            candle_type = "strong_bear"
            vote, strength = -1, 7
            reason = f"❤️ شمعة هابطة قوية | جسم={body_pct*100:.0f}%"

    # ════════════════════════════════════
    # ⑥ High Wave — ظلال طويلة
    # ════════════════════════════════════
    elif (upper_pct >= 0.30 and lower_pct >= 0.30 and body_pct <= 0.30):
        candle_type = "high_wave"
        vote, strength = 0, 3
        reason = "🌊 High Wave — تذبذب عالٍ وتردد"

    # ════════════════════════════════════
    # ⑦ Belt Hold
    # ════════════════════════════════════
    elif (bull and lower_pct < 0.03 and body_pct > 0.60):
        candle_type = "bull_belt_hold"
        vote, strength = 1, 7
        reason = f"📌 Belt Hold صاعد — افتتاح عند القاع"
    elif (not bull and upper_pct < 0.03 and body_pct > 0.60):
        candle_type = "bear_belt_hold"
        vote, strength = -1, 7
        reason = f"📌 Belt Hold هابط — افتتاح عند القمة"
    ...
    # ════════════════════════════════════
        ...
        if close_pos >= 0.75:
            candle_type = "upper_close"
            vote, strength = 1, 5
            reason = f"📈 إغلاق في الثلث العلوي ({close_pos*100:.0f}%)"
        elif close_pos <= 0.25:
            candle_type = "lower_close"
            vote, strength = -1, 5
            reason = f"📉 إغلاق في الثلث السفلي ({close_pos*100:.0f}%)"
        else:
            candle_type = "middle_close"
            vote, strength = 0, 2
            reason = f"↔️ إغلاق في المنتصف ({close_pos*100:.0f}%)"

    # ════════════════════════════════════
    # تعديلات حسب السياق
    # ════════════════════════════════════

    # حجم مرتفع → تعزيز القوة
    if vol_ratio >= 1.8:
        strength = min(10, strength + 2)
        reason += f" | 🔊 حجم×{vol_ratio:.1f}"
    elif vol_ratio <= 0.5:
        strength = max(0, strength - 2)
        reason += f" | 🔇 حجم ضعيف"

    # شمعة كبيرة مقارنة بـ ATR → تعزيز
    if size_vs_atr >= 1.5:
        strength = min(10, strength + 1)
        reason += f" | 📏 {size_vs_atr:.1f}×ATR"

    # كسر Kicker — شمعة عكسية تماماً
    if prev_candles and len(prev_candles) >= 1:
        prev = prev_candles[-1]
        prev_bull = float(prev.get("Close",0)) >= float(prev.get("Open",0))
        if bull != prev_bull and body_pct >= 0.70:
            strength = min(10, strength + 2)
            reason = "⚡ Kicker! " + reason
    ...
    return {
        ...
        "upper_pct":  round(upper_pct * 100, 1),
...
def analyze_single_candle() -> dict:
    """
    الوحدة الرئيسية — يحلل آخر شمعة مكتملة على 4 إطارات
    ويُرجع إشارة موحّدة مع تفاصيل كل إطار
    """
    _empty = {
        "vote": 0, "score": 0, "conf": 0,
        "signal_type": None, "entry": None, "sl": None,
        "tp1": None, "tp2": None, "rr": None,
        "reasons": [], "candles": {},
        "dominant": None, "agreement": 0,
    }
    ...
    for tf, period, interval, weight in frames:
        df = fetch(SYMBOL, period, interval)
        if df is None or len(df) < SC_CONTEXT_LOOKBACK + 2:
            continue
        if tf == "1h":
            df1h_ref = df

        # آخر شمعة مكتملة (وليس الشمعة الحالية)
        candle = df.iloc[-2]
        prev_5 = [df.iloc[i] for i in range(-7, -2)]

        # ATR و متوسط الحجم
        at_val  = float(atr_fn(df).iloc[-2]) if len(df) > SC_ATR_PERIODS else 5.0
        avg_vol = float(df["Volume"].rolling(SC_VOL_LOOKBACK).mean().iloc[-2]) or 1.0

        analysis = _analyze_single_candle(candle, prev_5, at_val, avg_vol)
        analysis["tf"] = tf
        ...
        if analysis["vote"] != 0:
            ...
            votes.append(analysis["vote"])
            reasons.append(f"[{tf}] {analysis['reason']}")

    if not votes:
        return {**_empty, "candles": results,
                "reasons": ["⏳ لا إشارة شمعية واضحة حالياً"]}

    # ── التصويت الموزون ──
    bull_score = sum(r["strength"] * r["weight"]
                     for r in results.values() if r.get("vote") == 1)
    bear_score = sum(r["strength"] * r["weight"]
                     for r in results.values() if r.get("vote") == -1)
    agree_bull = sum(1 for v in votes if v == 1)
    agree_bear = sum(1 for v in votes if v == -1)

    if bull_score > bear_score and agree_bull >= 2:
        final_vote = 1
        dominant_score = bull_score
    elif bear_score > bull_score and agree_bear >= 2:
        final_vote = -1
    ...
    agreement = max(agree_bull, agree_bear)
    conf      = min(88, int(dominant_score * 3 + agreement * 8))

    # ── الإشارة الأقوى ──
    dominant = max(results.values(),
                   key=lambda x: x.get("strength",0) * x.get("weight",1)
                   if x.get("vote") == final_vote else 0,
                   default=None)

    # ── SL/TP ──
    entry = sl = tp1 = tp2 = rr = None
    if final_vote and df1h_ref is not None:
        price  = float(df1h_ref["Close"].iloc[-1])
        at_1h  = float(atr_fn(df1h_ref).iloc[-1])
        entry  = price
        if final_vote == 1:
            sl  = round(price - at_1h * 1.5, 2)
            tp1 = round(price + at_1h * 2.0, 2)
            tp2 = round(price + at_1h * 3.5, 2)
        else:
            sl  = round(price + at_1h * 1.5, 2)
            tp1 = round(price - at_1h * 2.0, 2)
            tp2 = round(price - at_1h * 3.5, 2)
        rr = round(abs(tp2 - entry) / (abs(sl - entry) + 1e-9), 2)
        if rr < 1.3: final_vote = 0

    # ── نوع الإشارة ──
    signal_type = dominant.get("type","") if dominant else ""
    signal_names = {
        "bullish_marubozu": "Marubozu صاعد 🟩",
        "bearish_marubozu": "Marubozu هابط 🟥",
        "hammer":           "مطرقة 🔨",
        "shooting_star":    "نجمة راحلة ⭐",
        "dragonfly_doji":   "دوجي اليعسوب 🐉",
        "gravestone_doji":  "دوجي شاهد القبر 🪦",
        "strong_bull":      "شمعة صاعدة قوية 💚",
        "strong_bear":      "شمعة هابطة قوية ❤️",
        "bull_belt_hold":   "Belt Hold صاعد 📌",
        "bear_belt_hold":   "Belt Hold هابط 📌",
        "inverted_hammer":  "مطرقة معكوسة 🔄",
        "hanging_man":      "رجل مشنوق 🪝",
        "high_wave":        "High Wave 🌊",
        "upper_close":      "إغلاق علوي 📈",
        "lower_close":      "إغلاق سفلي 📉",
    }
    signal_display = signal_names.get(signal_type, signal_type)

    # ── إضافة سياق السوق ──
    if agreement >= 3:
        reasons.insert(0, f"✅ توافق {agreement}/4 إطارات على نفس الاتجاه")
    if dominant and dominant.get("size_vs_atr",0) >= 1.5:
        reasons.append(f"📏 شمعة {dominant['size_vs_atr']}× ATR — حركة قوية")

    log.info(f"[SingleCandle] vote={final_vote} score={score:.1f} "
             f"agree={agreement}/4 type={signal_type}")
    ...
    return {
        ...
        "entry":       round(entry, 2) if entry and final_vote else None,
        "sl":          round(sl, 2)    if sl    and final_vote else None,
        "tp1":         round(tp1, 2)   if tp1   and final_vote else None,
        "tp2":         round(tp2, 2)   if tp2   and final_vote else None,
        "rr":          rr              if final_vote else None,
        "reasons":     reasons[:6],
        "candles":     results,
        "dominant":    dominant,
        "agreement":   agreement,
...
# ══════════════════════════════════════════════════════════════════
#
# ① Smart Dedup     — تجنب الإشارات المتكررة بذكاء (بدل مقارنة السعر فقط)
# ② Signal Cooldown — فترة هدوء بين الإشارات (30 دقيقة)
# ③ Market Filter   — تصفية السوق الجانبي والأوقات الضعيفة
# ④ Streak Tracker  — تتبع سلسلة الفوز/الخسارة
# ⑤ Dynamic Agree   — تعديل UNI_AGR بحسب الأداء التاريخي

import time as _time
from collections import deque

# ── إعدادات ──
COOLDOWN_MIN      = 30    # دقائق هدوء بين الإشارات
SIDEWAYS_ADX_MAX  = 20    # ADX أقل من هذا = سوق جانبي
STREAK_LOSS_MAX   = 3     # بعد 3 خسائر متتالية → زِد المتطلبات
MARKET_HOURS_UTC  = [(7,17)]  # ساعات التداول الأفضل (7-17 UTC)
SIG_HISTORY_SIZE  = 50    # حجم سجل الإشارات

# ── سجل الإشارات الداخلي ──
_sig_history: deque = deque(maxlen=SIG_HISTORY_SIZE)
_last_sig_time: float = 0.0
_consecutive_losses: int = 0
_consecutive_wins:   int = 0


def _is_market_open_hours() -> bool:
    """هل نحن في أفضل ساعات التداول؟"""
    hour = utcnow().hour
    for start, end in MARKET_HOURS_UTC:
        if start <= hour < end:
            return True
    return False
...
def _adx_value(df) -> float:
    """يحسب قيمة ADX الحالية"""
    try:
        import ta
        adx = ta.trend.ADXIndicator(df["High"], df["Low"], df["Close"], window=14)
        return float(adx.adx().iloc[-1])
    except:
        return 25.0  # قيمة افتراضية تعني: السوق ليس جانبياً


def is_sideways_market(v7: dict) -> bool:
    """هل السوق جانبي؟ يستخدم ADX من V7"""
    # نتحقق من إطارات متعددة
    strats = v7.get("strats", {})
    adx_info = strats.get("V7_ADX", {})
    # إذا ADX ضعيف في أغلب الإطارات → سوق جانبي
    buy_w  = v7.get("buy_w",  0)
    sell_w = v7.get("sell_w", 0)
    max_w  = v7.get("max_weight", 22)
    # إذا لا يوجد توجه واضح في الأوزان
    if max(buy_w, sell_w) < max_w * 0.25:
        return True
    return False


def check_signal_cooldown() -> tuple:
    """
    يتحقق من فترة الهدوء بين الإشارات
    يُرجع: (ok, remaining_minutes)
    """
    global _last_sig_time
    elapsed = (_time.time() - _last_sig_time) / 60
...
def mark_signal_sent(direction: int, entry: float, conf: int):
    """يُسجّل إرسال إشارة جديدة"""
    global _last_sig_time
    _last_sig_time = _time.time()
    _sig_history.append({
        "ts":    utcnow_str(),
        "dir":   direction,
        "entry": entry,
        "conf":  conf,
        "result": None  # يُحدَّث لاحقاً
    })


def update_streak(result: str):
    """يُحدّث سلسلة الفوز/الخسارة"""
    global _consecutive_losses, _consecutive_wins
    if result == "win":
...
def get_dynamic_min_agree() -> int:
    """
    يُعدّل الحد الأدنى للتوافق بحسب الأداء:
    - بعد سلسلة خسائر → اشترط توافقاً أعلى
    - أداء جيد → يبقى عادياً
    """
    base = UNI_AGR
    if _consecutive_losses >= STREAK_LOSS_MAX:
        new_agree = min(base + 2, 7)
        log.info(f"[Perf] {_consecutive_losses} خسائر متتالية → UNI_AGR مؤقت={new_agree}")
        return new_agree
    return base


def get_signal_stats() -> dict:
    """إحصائيات سريعة من سجل الإشارات الأخيرة"""
    hist = list(_sig_history)
    if not hist: return {"total":0,"win_rate":"—"}
    resolved = [s for s in hist if s.get("result")]
    ...
    return {
        ...
        "wins":     wins,
        "win_rate": f"{wr:.1f}%",
        "streak_w": _consecutive_wins,
        "streak_l": _consecutive_losses,
    }


def should_send_signal(agg: dict, sq_res: dict, v7: dict) -> tuple:
    """
    يُقرر هل يُرسل الإشارة بناءً على كل العوامل
    يُرجع: (send: bool, reason: str)
    """
    # 1. فحص جودة الإشارة
    if not sq_res.get("tradeable", False):
        return False, f"⚠️ جودة ضعيفة {sq_res.get('score',0)}/100"

    # 2. فحص فترة الهدوء
    cd_ok, cd_rem = check_signal_cooldown()
    if not cd_ok:
        return False, f"⏳ فترة هدوء — {cd_rem:.0f} دقيقة متبقية"

    # 3. فحص التوافق الديناميكي
    dyn_agree = get_dynamic_min_agree()
    if agg.get("agree", 0) < dyn_agree:
        return False, f"⚠️ توافق {agg.get('agree',0)} < {dyn_agree} المطلوب"

    # 4. فحص السوق الجانبي
    if is_sideways_market(v7):
        return False, "⚠️ سوق جانبي — تجنّب الدخول"

    # 5. ساعات التداول (تحذير فقط — لا يمنع)
    if not _is_market_open_hours():
        log.warning("[Perf] خارج أفضل ساعات التداول")
...
# ══════════════════════════════════════════════════════════════
# CLAUDE_PROVIDER في config.json:
#   "auto"       ← يجرّب المجاني أولاً ثم المدفوع (موصى به)
#   "groq"       ← مجاني تماماً (14,400 طلب/يوم) — الأسرع
#   "openrouter" ← مجاني (200 طلب/يوم) — أكثر تنوعاً
#   "gemini"     ← مجاني (250 طلب/يوم) — من Google
#   "ppq"        ← مدفوع بـ USDT
#   "anthropic"  ← مدفوع بطاقة بنكية
# ══════════════════════════════════════════════════════════════

_AI_PROVIDERS = {
    # ── مجانية ──────────────────────────────────────────
    "groq": {
        # مجاني — 14,400 طلب/يوم — الأسرع — بدون بطاقة
        # سجّل على: console.groq.com
        "url":   "https://api.groq.com/openai/v1/chat/completions",
        "model": cfg.get("CLAUDE_MODEL","") or "llama-3.3-70b-versatile",
        "fmt":   "openai",
        "headers": lambda key: {
    ...
    "openrouter": {
        # مجاني — 200 طلب/يوم — 29+ نموذج مجاني
        # سجّل على: openrouter.ai
        "url":   "https://openrouter.ai/api/v1/chat/completions",
        "model": cfg.get("CLAUDE_MODEL","") or "meta-llama/llama-3.3-70b-instruct:free",
        "fmt":   "openai",
        "headers": lambda key: {
            "Authorization":  f"Bearer {key}",
            "Content-Type":   "application/json",
            "HTTP-Referer":   "https://github.com/goldbot",
            "X-Title":        "Gold Trading Bot",
        },
    },
    "gemini": {
        # مجاني — 250 طلب/يوم — من Google
        # سجّل على: ai.google.dev
        "url":   "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",
        "model": cfg.get("CLAUDE_MODEL","") or "gemini-2.0-flash",
        "fmt":   "openai",
        "headers": lambda key: {
            "Authorization": f"Bearer {key}",
            "Content-Type":  "application/json",
...
# ── ترتيب Auto: مجاني أولاً ──
_AUTO_ORDER = ["groq", "openrouter", "gemini", "ppq", "anthropic"]


def _ai_provider() -> tuple:
    """يعيد إعدادات المزوّد المختار من config.json"""
    provider = cfg.get("CLAUDE_PROVIDER", "auto").lower().strip()
    if provider == "auto":
        # في وضع auto، نُرجع قائمة للتجربة
        return None, "auto"
    if provider not in _AI_PROVIDERS:
        log.warning(f"[AI] مزوّد غير معروف '{provider}' — استخدام groq")
        provider = "groq"
...
def _build_ai_prompt(v7: dict, smc: dict, ob: dict, agg: dict, price: float) -> str:
    ...
    prompt = f"""You are an expert gold (BTC/USD) technical analyst with 20+ years of experience.
...
def ai_analyze(v7: dict, smc: dict, ob: dict, agg: dict, price: float) -> dict:
    """
    يرسل بيانات التحليل إلى AI ويعيد قراره النهائي.
    يدعم Auto (مجاني أولاً) + groq + openrouter + gemini + ppq + anthropic
    """
    _empty = {
    ...
    provider = cfg.get("CLAUDE_PROVIDER","auto").strip().lower()
    if not api_key:
        log.debug("[AI] CLAUDE_API_KEY غير مضبوط")
        return _empty

    prompt = _build_ai_prompt(v7, smc, ob, agg, price)

    def _try_provider(pname: str) -> tuple:
        """يجرّب مزوّداً ويُرجع (raw_text, provider_name) أو (None, pname)"""
        pcfg = _AI_PROVIDERS.get(pname)
        if not pcfg: return None, pname
        try:
            resp = requests.post(
                pcfg["url"],
                headers=pcfg["headers"](api_key),
                json={"model":pcfg["model"],"max_tokens":1024,
                      "messages":[{"role":"user","content":prompt}]},
                timeout=30,
            )
            if resp.status_code == 429:
                log.warning(f"[AI] {pname}: تجاوز الحد (429)")
                return None, pname
            if not resp.ok:
                log.debug(f"[AI] {pname}: HTTP {resp.status_code}")
                return None, pname
            data = resp.json()
            if pcfg["fmt"] == "anthropic":
                text = data.get("content",[{}])[0].get("text","").strip()
            else:
                text = data.get("choices",[{}])[0].get("message",{}).get("content","").strip()
            return (text if text else None), pname
        except Exception as e:
            log.debug(f"[AI] {pname} خطأ: {e}")
            return None, pname

    # ── اختيار المزوّد ──
    if provider == "auto":
        providers_to_try = _AUTO_ORDER
    elif provider in _AI_PROVIDERS:
        providers_to_try = [provider]
    else:
        log.warning(f"[AI] مزوّد غير معروف '{provider}' — استخدام auto")
        providers_to_try = _AUTO_ORDER

    raw_text = None
    used_provider = "—"
    for pname in providers_to_try:
        log.info(f"[AI] جرّب {pname}...")
        raw_text, used_provider = _try_provider(pname)
        if raw_text:
            log.info(f"[AI] ✅ {pname} أجاب")
            break
        log.debug(f"[AI] {pname} فشل — التالي")

    if not raw_text:
        log.warning("[AI] جميع المزوّدين فشلوا")
        return {**_empty, "provider": used_provider}
    ...
    try:
        ...
        log.info(f"[AI] ✅ [{used_provider}] قرار={dec} ثقة={conf}% توافق={agree}")
        return result
    except json.JSONDecodeError:
        # النموذج لم يُرجع JSON — حوّله لنتيجة بسيطة
        log.debug(f"[AI] {used_provider} لم يُرجع JSON — استخدم النص مباشرة")
        return {**_empty, "available":True, "provider":used_provider,
                "summary_ar": raw_text[:500], "decision":"WAIT", "confidence":50}
    except Exception as e:
        log.warning(f"[AI] خطأ: {e}")
        return {**_empty, "provider": used_provider}
...
def _ai_section(ai: dict, sys_vote: int) -> str:
    ...
    dec    = ai.get("decision","?")
    prov   = ai.get("provider","auto").upper()
...
# AGGREGATOR
# ══════════════════════════════════════════════════════════════════
def aggregate(v7: dict, smc: dict, ob: dict, asia: dict = None, rng: dict = None, fib: dict = None, rev: dict = None, sc: dict = None) -> dict:
    if asia is None: asia = {}
    if rng  is None: rng  = {}
    if fib is None: fib = {}
    if rev  is None: rev  = {}
    if sc   is None: sc   = {}
    votes={"V7":v7["vote"],"SMC":smc["vote"],"OBinOB":ob["vote"],"Asia":asia.get("vote",0),"Range":rng.get("vote",0),"Fib":fib.get("vote",0),"Rev":(rev.get("vote",0) if rev else 0),"SC":(sc.get("vote",0) if sc else 0)}
    ...
    if d:
        ...
        if d==fib.get("vote",0) and fib.get("entry"): conf+=int(fib.get("conf",0)*0.12)
    ...
    if d:
        ...
        if os_ and os_.side==d:
            ...
            tp=tp1; rr=rng["rr"]
        elif fib.get("vote")==d and fib.get("entry"):
            entry=fib["entry"]; sl=fib["sl"]
            tp1=fib.get("tp1"); tp2=fib.get("tp2"); tp3=fib.get("tp3")
            tp=tp1 or fib["entry"]; rr=fib.get("rr")
        elif rev and rev.get("vote")==d and rev.get("entry"):
            entry=rev["entry"]; sl=rev["sl"]
            tp1=rev.get("tp1"); tp2=rev.get("tp2"); tp3=tp2
            tp=tp1 or rev["entry"]; rr=rev.get("rr")
        elif sc and sc.get("vote")==d and sc.get("entry"):
            entry=sc["entry"]; sl=sc["sl"]
            tp1=sc.get("tp1"); tp2=sc.get("tp2"); tp3=tp2
            tp=tp1 or sc["entry"]; rr=sc.get("rr")
    ...
    return {"d":d,"agree":ag_cnt,"conf":conf,"votes":votes,"lbl":lbl,
            "entry":entry,"sl":sl,"tp":tp,"tp1":tp1,"tp2":tp2,"tp3":tp3,"rr":rr,
            "asia":asia,"range":rng,"fib":fib,"rev":rev,"sc":sc}
...
def build_chart(v7: dict, smc: dict, ob: dict, agg: dict) -> Optional[str]:
    ...
    ap.set_title(f"BTC/USD  {agg['lbl']}  ثقة={agg['conf']}%  [{vt}]  {trading_session()}  {utcnow_str()}",
...
def _save_results(v7, smc, ob, asia, rng, agg, price, fib=None, rev=None, sc=None):
    global _last_results, _last_results_ts
    _last_results = {"v7":v7,"smc":smc,"ob":ob,"asia":asia,"range":rng,"agg":agg,"price":price,"fib":fib or {},"rev":rev or {},"sc":sc or {}}
...
def _build_keyboard(signal_dir: int = 0) -> dict:
    ...
    return {"inline_keyboard":[
        ...
        [btn("📐 Range تفصيلي","analyze_range"),  btn("📊 فيبوناتشي","analyze_fib")],
        [btn("🔄 أنماط الانعكاس","analyze_rev"),     btn("🕯️ الشمعة الواحدة","analyze_sc")],
        [btn("🤖 Claude AI","analyze_ai"),           btn(f"📈 ملخص {arrow}","analyze_full")],
    ]}
...
def _call_ai_api(url: str, headers: dict, model: str, fmt: str, prompt: str) -> str:
    """يستدعي AI API ويُرجع النص أو None عند الفشل"""
    try:
        payload = {"model": model, "max_tokens": 1500,
                   "messages": [{"role":"user","content": prompt}]}
        resp = requests.post(url, headers=headers, json=payload, timeout=40)
        if not resp.ok:
            log.debug(f"[AI] HTTP {resp.status_code}: {resp.text[:100]}")
            return None
        data = resp.json()
        if fmt == "anthropic":
...
def _ai_deep_analysis(strat: str, results: dict) -> str:
    ...
    provider = cfg.get("CLAUDE_PROVIDER","auto").strip().lower()
    if not api_key:
        return "⚠️ أضف AI_API_KEY في الإعدادات — راجع /about لمزوّدين مجانيين"

    prompt = _make_prompt(strat, results, results.get("price",0))

    if provider == "auto":
        # جرّب كل مزوّد بالترتيب
        for pname in _AUTO_ORDER:
            pcfg = _AI_PROVIDERS[pname]
            log.info(f"[AI-Auto] جرّب {pname}...")
            result = _call_ai_api(
                pcfg["url"], pcfg["headers"](api_key),
                pcfg["model"], pcfg["fmt"], prompt
            )
            if result:
                log.info(f"[AI-Auto] ✅ نجح {pname}")
                return result
            log.debug(f"[AI-Auto] فشل {pname} — جرّب التالي")
        return "❌ فشلت جميع المزوّدين — تحقق من صلاحية المفتاح"

    # مزوّد محدد
    prov_cfg = _AI_PROVIDERS.get(provider, _AI_PROVIDERS["groq"])
    url      = prov_cfg["url"]
...
def _quick_local(data: str, results: dict) -> str:
    ...
    if data=="analyze_sc":
        sc_=results.get("sc",{}) or {}
        L=["<b>تحليل الشمعة الواحدة — "+str(sc_.get("agreement",0))+"/4 إطارات توافق</b>"]
        L.append("النوع: "+esc(str(sc_.get("signal_type","—"))))
        for tf,cd in sc_.get("candles",{}).items():
            ic="🟢" if cd.get("vote")==1 else ("🔴" if cd.get("vote")==-1 else "⬜")
            L.append(ic+" ["+tf+"]: "+esc(cd.get("reason","?")[:55]))
            L.append("   جسم="+str(cd.get("body_pct","?"))+"% | ظل↑="+str(cd.get("upper_pct","?"))+"% | ظل↓="+str(cd.get("lower_pct","?"))+"% | إغلاق="+str(cd.get("close_pos","?"))+"%")
        for rsn in sc_.get("reasons",[])[:4]: L.append(esc(rsn))
        return "\n".join(L)
    if data=="analyze_rev":
        rev_=results.get("rev",{}) or {}
        pats=rev_.get("patterns",[])
        L=["<b>الأنماط ("+str(len(pats))+"):</b>"]
        for p in pats[:6]:
            ic="🟢" if p.get("vote")==1 else ("🔴" if p.get("vote")==-1 else "⬜")
            L.append(ic+" ["+p.get("tf","?")+"] "+esc(p.get("name","?")[:50]))
        for rsn in rev_.get("reasons",[])[:4]: L.append(esc(rsn))
        return "\n".join(L)
    if data=="analyze_fib":
        fib_=results.get("fib",{}) or {}
        wave_=fib_.get("wave",{}) or {}
        gp_=fib_.get("gp",{}) or {}
        ret_=fib_.get("retrace",{}) or {}
        ext_=fib_.get("extend",{}) or {}
        pvt_=fib_.get("pivot",{}) or {}
        L=["<b>الموجة:</b> "+("UP" if wave_.get("direction")==1 else "DN")+" "+str(wave_.get("size_pct","?"))+"%  Hi="+str(wave_.get("wave_hi","?"))+" Lo="+str(wave_.get("wave_lo","?"))]
        L.append("<b>التصحيح:</b> "+" | ".join(k+"="+str(v) for k,v in ret_.items()))
        L.append("<b>الامتداد:</b> "+" | ".join(k+"="+str(v) for k,v in list(ext_.items())[:4]))
        if gp_: L.append("<b>الجيب الذهبي:</b> "+str(gp_.get("bottom","?"))+"--"+str(gp_.get("top","?")))
        if pvt_: L.append("<b>Pivot:</b> P="+str(pvt_.get("P","?"))+" R1="+str(pvt_.get("R1","?"))+" S1="+str(pvt_.get("S1","?")))
        for rsn in fib_.get("reasons",[])[:5]: L.append(esc(rsn))
        return "\n".join(L)
    votes=agg.get("votes",{})
...
def handle_callback(callback_query: dict):
    ...
    names = {
        ...
        "analyze_sc":"🕯️ الشمعة الواحدة",
        "analyze_rev":"🔄 أنماط الانعكاس",
        "analyze_fib":"📊 فيبوناتشي الشامل",
...
def _pu_prime_ad(signal_dir: int = 0) -> str:
    ...
    _pair = cfg.get("SYMBOL", "BTC-USD")
    _name = {"BTC-USD":"الذهب","BTC-USD":"البيتكوين",
...
def msg_signal(v7: dict, smc: dict, ob: dict, agg: dict, trigger_reasons: list, sq: dict = None, cm: dict = None) -> str:
    ...
    lines=[
        f"{'📈' if d==1 else '📉'} <b>إشارة البيتكوين موحّدة — BTC/USD</b>",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
    ...
    lines.append(f"\n#Bitcoin #BTC_USD #إشارات_بيتكوين #تحليل_فني")
    if sq and cm:
        lines.append(msg_quality_section(sq, cm, d))
    lines.append(_pu_prime_ad(d))
    return "\n".join(lines)

def msg_analysis(v7: dict, smc: dict, ob: dict, price: float, asia: dict = None, rng: dict = None, fib: dict = None, sc: dict = None) -> str:
    ...
    lines+=[
        "",f"⚙️ الإشارة عند توافق {UNI_AGR}+ من 3  |  #Bitcoin"
    ]
...
def main():
    log.info("="*65)
    log.info("🚀 بوت البيتكوين الموحّد — يبدأ التشغيل")
    ...
    if tg_ok:
        tg_send(
            "🟠 <b>بوت البيتكوين الموحّد — انطلق الآن!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    perf=load_perf(); last_sig=None; last_rpt=None; cycle=0; cur_price=65000.0
    ...
    while True:
        ...
        log.info("⑧ Single Candle...")
        _sc0={"vote":0,"score":0,"conf":0,"signal_type":None,"entry":None,"sl":None,"tp1":None,"tp2":None,"rr":None,"reasons":[],"candles":{},"dominant":None,"agreement":0}
        try:    sc_res=analyze_single_candle()
        except: log.exception("SC error"); sc_res=_sc0

        log.info("⑦ Reversal Patterns...")
        _rv={"vote":0,"score":0,"conf":0,"patterns":[],"strongest":None,"entry":None,"sl":None,"tp1":None,"tp2":None,"rr":None,"signal_type":None,"reasons":[]}
        try:    rev_res=analyze_reversal()
        except: log.exception("Rev error"); rev_res=_rv

        # ── Fibonacci ──
        log.info("⑥ Fibonacci...")
        _fe={"vote":0,"score":0,"conf":0,"signal_type":None,"wave":None,"retrace":{},"extend":{},"pivot":{},"gp":{},"confluence":[],"entry":None,"sl":None,"tp1":None,"tp2":None,"tp3":None,"rr":None,"reasons":[],"levels_1h":{},"levels_4h":{}}
        try:    fib_res=analyze_fibonacci()
        except: log.exception("Fib error"); fib_res=_fe
        agg=aggregate(v7,smc,ob,asia,rng_res,fib_res,rev_res,sc_res)
        ...
        _save_results(v7,smc,ob,asia,rng_res,agg,smc.get("price",cur_price),fib_res,rev_res,sc_res)

        # ── جودة الإشارة ──
        _at_df = fetch(SYMBOL,"7d","1h")
        _at_v  = float(atr_fn(_at_df).iloc[-1]) if _at_df is not None and len(_at_df)>1 else 5.0
        sq_res = calc_signal_quality(agg,v7,smc,ob.get("signal"),fib_res,rev_res,rng_res)
        cm_res = calc_counter_move_plan(agg,smc.get("price",cur_price),_at_v)
        log.info(f"[SQ] {sq_res['score']}/100 {sq_res['grade']} — {'PASS' if sq_res['tradeable'] else 'SKIP'}")
        ...
        if agg["d"]!=0 and agg["entry"] is not None:
            ...
            if sid==last_sig:
                ...
                if fib_res.get("vote")==agg["d"] and fib_res.get("entry"):
                    _fst="Fib_GoldenPocket" if "ذهبي" in str(fib_res.get("signal_type","")) else "Fib_Retracement"
                    rec_sig(perf,_fst,agg["d"],agg["entry"],agg["sl"],agg.get("tp",agg["entry"]),ts)
                if rev_res.get("vote")==agg["d"] and rev_res.get("entry"):
                    _rst="Rev_HeadShoulders" if "رأس" in str(rev_res.get("signal_type","")) else "Rev_Engulfing"
                    rec_sig(perf,_rst,agg["d"],agg["entry"],agg["sl"],agg.get("tp",agg["entry"]),ts)
                if sc_res.get("vote")==agg["d"] and sc_res.get("entry"):
                    _sct="SC_Marubozu" if "marubozu" in str(sc_res.get("dominant",{}).get("type","")) else ("SC_Hammer" if "hammer" in str(sc_res.get("dominant",{}).get("type","")) else ("SC_Doji" if "doji" in str(sc_res.get("dominant",{}).get("type","")) else "SC_Strong"))
                    rec_sig(perf,_sct,agg["d"],agg["entry"],agg["sl"],agg.get("tp",agg["entry"]),ts)

                send_ok, send_reason = should_send_signal(agg, sq_res, v7)
                if not send_ok:
                    log.info(f"[Filter] إشارة مُصفَّاة: {send_reason}")
                    continue
                mark_signal_sent(agg["d"], agg.get("entry",0), agg.get("conf",0))
                txt=msg_signal(v7,smc,ob,agg,reasons,sq_res,cm_res)
                try:
            ...
            ana_txt=msg_analysis(v7,smc,ob,smc.get("price",cur_price),asia,rng_res,fib_res,sc_res)
